...
public class LineSection extends LineAG implements Cloneable {
    ...
    public PointAG computeCenter() {
        float centerX = (p1.getX() + p2.getX()) / 2;
        float centerY = (p1.getY() + p2.getY()) / 2;
    ...
    public double computeInclinationAngle() {
        double ySpan = this.p1.getY() - this.p2.getY();
        double xSpan = this.p1.getX() - this.p2.getX();
        return Math.atan2(xSpan, ySpan);
    }

    public final Function<Float, Boolean> xBelongsToLineSection = x
            -> (x >= Math.min(this.p1.getX(), this.p2.getX()) && (x <= Math.max(this.p1.getX(), this.p2.getX())));

    public final Function<Float, Boolean> yBelongsToLineSection = y
            -> (y >= Math.min(this.p1.getY(), this.p2.getY()) && (y <= Math.max(this.p1.getY(), this.p2.getY())));
    ...
    private PointAG returnIntersectionIfInLineSection(PointAG intersection) {
        if (intersection != null) {
            if (this.xBelongsToLineSection.apply(intersection.getX())
                    && this.yBelongsToLineSection.apply(intersection.getY()))
                return intersection;
            else return null;
        } else
            return null;
    ...
    public final Function<Float, LineSection> createPararellSection = distance -> {
        ...
        PointAG parSectionP1 = perpLineP1.givenXMovePointAlongLine(this.p1.getX(), distance);
        PointAG parSectionP2 = perpLineP2.givenXMovePointAlongLine(this.p2.getX(), distance);

        return new LineSection(parSectionP1, parSectionP2);
    };

    public static final Function<LineSection, Double> computeLength = section
            -> Math.sqrt(Math.pow((section.p1.getY() - section.p2.getY()), 2)
            + Math.pow((section.p1.getX() - section.p2.getX()), 2));
    ...
    public String toString() {
        return "(" + this.p1.getX() + ", " + this.p1.getY()
